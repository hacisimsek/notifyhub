...
The dashboard supports register, login, current-user lookup, profile updates and password changes through the Gateway API. Registration asks for first name, last name and phone number in addition to email and password. Signed-in users manage editable profile details and password changes from the Profile page; successful profile or password changes refresh the stored bearer token without signing the user out.
